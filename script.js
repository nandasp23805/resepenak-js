...
async function loadDataResep() {
    ...
    container.innerHTML = ""; // Bersihkan kontainer sebelum memuat ulang
    ...
    data.forEach(item => {
        ...
        col.innerHTML = `
            <div class="card bg-light h-100">
                <div class="card-body">
                    <h5 class="card-title" data-bs-toggle="collapse" data-bs-target="#collapse${item.id}" aria-expanded="false" aria-controls="collapse${item.id}" style="cursor:pointer;">
                        ${item.judul}
                    </h5>
    ...
    collapseElements.forEach(element => {
        if (bootstrap.Collapse.getInstance(element)) {
            bootstrap.Collapse.getInstance(element).hide(); // Pastikan tertutup
        } else {
            new bootstrap.Collapse(element, { toggle: false }); // Inisialisasi jika belum, dan jangan langsung buka
        }
    });
}
...
function showPage(page) {
    ...
    // Kelola kelas body untuk footer
    if (page === 'home') {
